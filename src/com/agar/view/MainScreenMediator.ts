...
module agar {
    ...
    export class MainScreenMediator extends puremvc.Mediator implements puremvc.IMediator
    {
        public static NAME : string = "MainScreenMediator";
        public NEW_X: number;
        public NEW_Y: number;
        public ROUND_RAD: number;
        private gameProxy: GameProxy;
        private foodProxy: FoodProxy;
        public next_x: number;
        public next_y: number;
        public time: number;//时间间隔
        public num: number=0;//分离标志
        public f: number;//分裂后的第几个圆
        public h:number;//合并的第几个圆
        public he_bing: boolean=false;//合并标志
        public fenlie_next_x: number;
        public fenlie_next_y: number;
        public hebing_user: boolean = false;
        public constructor(viewComponent : any)
        {
            super(MainScreenMediator.NAME, viewComponent);           
            this.gameProxy = <GameProxy><any>(this.facade.retrieveProxy(GameProxy.NAME));
            this.foodProxy = <FoodProxy><any>(this.facade.retrieveProxy(FoodProxy.NAME));
        ...
        public get view() : MainScreen
        {
            return <MainScreen><any> (this.viewComponent);
        }
        ...
        {
            return [
            GameProxy.ROUND_MOVE,
            GameProxy.EAT,
            GameProxy.USER_NAME,
            GameProxy.FENLIE_MOVE,
            GameProxy.HEBING_USER,
            GameProxy.FEN,
            GameProxy.FOOD_EAT
            ];
        }       
        ...
        public handleNotification(notification : puremvc.INotification) : void
        {
            ...
            switch(notification.getName()){
                   ...
                   // this.foodMove();
                    for(var j = 0;j < this.gameProxy.getUserNum();++j) {
                        ...
                        if(this.gameProxy.getUserData()[j][2] > this.gameProxy.getHideRad()) {
                          this.sendNotification(GameProxy.ROUND_HIDE,j);
                        }
                        this.sendNotification(GameProxy.ROUND_EAT,j);
                    }
                ...
                case GameProxy.FENLIE_MOVE: {
                    ...
                    this.sendNotification(GameProxy.ROUND_EAT,i);
               ...
               case GameProxy.FOOD_EAT:{
                             //机器人吃food
                          var i = data[0],i_x=this.gameProxy.getFood()[i][0],i_y=this.gameProxy.getFood()[i][1],
                          i_r=this.gameProxy.getFood()[i][2],i_n=this.gameProxy.getFood()[i][3];
                      ...
                      var x1 = Math.random() * 4500,y1 = Math.random() * 7000,
                     angleSpeed = Math.atan2(y1 - i_y,x1 - i_r);
                ...
                case GameProxy.HEBING_USER: {
                    ...
                    var next_x = user_data[i][0] + vx;
                    var next_y = user_data[i][1] + vy;
                    var next:egret.Point = new egret.Point(next_x,next_y);
                    this.gameProxy.setUserDataCenter(i,next_x,next_y);
                    this.sendNotification(GameProxy.ROUND_EAT,i);
                    ...
                    if( l< user_data[0][2]+user_data[i][2]+20&&!this.he_bing) {
                        ...
                        if(l < 2) {                           
                                ...
                                if(this.gameProxy.getUserNum() > 1) {
                                    this.sendNotification(GameProxy.HEBING,i);
                                }
                            this.he_bing = false;
        ...
        public onEnterFrame() {            
            if(this.hebing_user) {
                this.hebing_user = false;
                this.sendNotification(GameProxy.HEBING,this.h);
                
            } 
            if(0<this.next_x&&this.next_x<4800&& 0<this.next_y&&this.next_y<8000) {
                this.sendNotification(GameProxy.NEXT_CENTER);                
           }
            if((this.num < 25&&this.num>0)) {
                    this.sendNotification(GameProxy.FENLIE,this.f);
            }
            else {
                if(this.num >= 25) {                
        ...
        public send() {
            ...
            for(var k = this.gameProxy.getUserNum()-1;k >0;k--) {
                ...
                this.sendNotification(GameProxy.HEBING,k);
            }
        }
        public hebing(k:number) {
               var user_data = this.gameProxy.getUserData();
        ...
        private init_view() : void
        {
             ...
             for(var i = 0;i <1000;++i){
                var x1:number=Math.random()*4800;
                var y1:number=Math.random()*8000;
            ...
            for(var j=0;j<hide;++j){
                var x2:number=Math.random()*4700;
                var y2:number=Math.random()*7900;
        ...
        public angleView(cen_x:number,cen_y:number) {
        ...
        public fen(evt:egret.TouchEvent){
            console.log("分离");
      ...
      //      }     
        ...
        public newUser(k:number){//k表示从哪一个圆分裂出来
            ...
            this.view.addUser(i + 1);
                ...
                this.gameProxy.setUserNum(this.gameProxy.getUserNum() + 1);
                 ...
                 this.sendNotification(GameProxy.FENLIE,i + 1);

        }
        public newCenter(event) {
            var NEW_X =event.stageX-this.view.map.x;
            var NEW_Y =event.stageY-this.view.map.y;
            
             if(NEW_X < 0) this.NEW_X = 0;
             if(NEW_X > 4800) this.NEW_X = 4800;
             if(NEW_Y < 0) this.NEW_Y = 0;
             if(NEW_Y > 8000) this.NEW_Y = 8000;
           ...
           // var i = 1000;
            for(var i = 1000;i < 1100;i++) {
                ...
                //this.ai(x,y,i);
                   ...
                   var x1 = Math.random() * 4000,y1 = Math.random() * 5000,
                     angleSpeed = Math.atan2(y1 - y,x1 - x);
                     angleSpeed = Math.round(angleSpeed * 10000) / 10000;
                 
                  this.gameProxy.getFood()[i][4] = angleSpeed;
                  //this.sendNotification(GameProxy.FOOD_MOVE,[i,vx,vy]);                    
                    }
        ...
        public foodMove() {
            for(var k = 1000;k < this.gameProxy.getFood().length;k++) {
                ...
                var vx: number = Math.round(Math.cos(angleSpeed) * 3 * 10000) / 10000;
                    var vy: number = Math.round(Math.sin(angleSpeed) * 3 * 10000) / 10000;         
                        var new_x=this.gameProxy.getFood()[k][0]+vx,
                        new_y=this.gameProxy.getFood()[k][1]+vy,
                        new_r=this.gameProxy.getFood()[k][2];
                        if(new_x<10)new_x=10;
                        if(new_x>4700)new_x=4700;
                        if(new_y<10)new_y=10;
                        if(new_y>7900)new_y=7900; 
                     ...
                     this.sendNotification(GameProxy.ROUND_EAT,k);
                    }
       ...
       public ai(){
        for(var i=1000;i<this.gameProxy.getFood().length;i++){
            if(this.gameProxy.getFood()[i]!=0){
                ...
                var x1 = Math.random() * 4500,y1 = Math.random() * 7000,
                    x=this.gameProxy.getFood()[i][0],y=this.gameProxy.getFood()[i][1],
                     angleSpeed = Math.atan2(y1 - y,x1 - x);
                     angleSpeed = Math.round(angleSpeed * 10000) / 10000;
                  this.gameProxy.getFood()[i][4] = angleSpeed;
            }
        //    egret.setTimeout(this.ai, this, 5000);
        }
